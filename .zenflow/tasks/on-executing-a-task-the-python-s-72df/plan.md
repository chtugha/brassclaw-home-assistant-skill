...
### [x] Step: Full codebase audit — dual-mode system and installer correctness
- **Re-read entire codebase**: All Rust source (lib.rs, api.rs, types.rs — 985+328+161 lines), WIT interface, all 4 install scripts (local/remote/build/duckdns), all 4 SKILL.md/HEARTBEAT.md/routines.md template files (local+remote), README.md, .gitignore
- **Read IronClaw http tool source** (`src/tools/builtin/http.rs` — ~700 lines from GitHub): Confirmed `headers` parameter accepts both array-of-objects `[{"name":"...","value":"..."}]` and flat key-value `{"Header":"value"}` formats. Local SKILL.md uses array format — matches IronClaw schema.
- **Rust code audit result**: No bugs. All validation, URL parsing, SSRF checks, entity/domain/service validators, date parsing, Modbus value type checks, template truncation, compact entity projection are correct. 39+ unit tests covering all validators.
- **CRITICAL bug fixed — variant mismatch on installer switch**: Both local and remote heartbeat/routines templates install to the same destination (`~/.ironclaw/HEARTBEAT.md`, `~/.ironclaw/routines.md`). Both installers previously skipped if file existed, meaning switching from remote→local (or vice versa) left the wrong variant in place (e.g., local heartbeat referencing `http`/`shell` tools when `ha-tool` was installed). Fixed by adding `<!-- VARIANT: local -->` / `<!-- VARIANT: remote -->` markers to all 4 template files and adding variant detection to both installers — wrong-variant files are now auto-replaced.
- **UX bug fixed — remote installer local URL warning too late**: Local URL detection appeared at the end of the remote installer (after the expensive WASM build step 3). Moved to right after step 2 (URL prompt) with an interactive abort prompt, saving the user 1-2 minutes of unnecessary compilation. Removed the duplicate warning from the end of the script.
- **Consistency fix — missing routines source-not-found warning**: Remote installer had no `warn` message when `routines.md` source was missing. Added `warn "No routines.md found — skipping."` to match the heartbeat block pattern.
- **Summary text fix**: SKILL_STATUS summary said "skipped — already exists" for the "already up to date" case. Changed to "skipped — already up to date" in both installers.
- **Dual-mode system verification**: Mutual exclusion (skill removal) works correctly in both directions. URL/token storage paths are correct. Variant markers ensure heartbeat/routines consistency across installer switches.
- **Bash syntax validated**: All 4 scripts pass `bash -n` clean
